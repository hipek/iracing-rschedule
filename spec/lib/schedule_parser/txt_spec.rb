...
describe ScheduleParser::Txt do
  describe "2014S3" do
    before(:all) do
      @series = described_class.new(
        fixture_file_upload('files/2014S3.txt')
      ).series
    end
  ...
  describe "2014S4" do
    before(:all) do
      @series = described_class.new(
        fixture_file_upload('files/2014S4.txt')
      ).series
    end
    ...
    it "has license" do
      expect(@series.map(&:license).sort.uniq).to eql(%w'A B C D P R')
    end
